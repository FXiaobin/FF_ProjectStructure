...
static CGFloat lineBottomMargin = 5.0;
...
-(void)setLineHeight:(CGFloat)lineHeight{
    _lineHeight = lineHeight >= 0.0 ? lineHeight : 1.0;
    _lineView.frame = CGRectMake(_titleButtonWidth/4, self.titleScrollHeight - lineHeight - lineBottomMargin, _titleButtonWidth/2, lineHeight);
    
    _lineView.clipsToBounds = YES;
    _lineView.layer.cornerRadius = lineHeight / 2.0;
}
...
- (void)configUI {
    // 最下面添加一条阴影
    CGRect frame = CGRectMake(0, 0, CGRectGetWidth([UIScreen mainScreen].bounds), 50.0 + 3.0);
    UIImageView *lineView = [[UIImageView alloc] initWithFrame:frame];
    lineView.image = [UIImage imageNamed:@"tag_bg"];
    [self.titleScroll addSubview:lineView];
    [self.titleScroll sendSubviewToBack:lineView];
}
...
- (void)collectionView:(UICollectionView *)collectionView willDisplayCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (![cell.contentView.subviews containsObject:childVC.view]) {
        ...
        [self.targetVC addChildViewController:childVC];
    }
}
//将要加载头尾视图时调用的方法
...
- (void)animationItem:(BOOL)isleft percent:(CGFloat)percent index:(NSInteger)index{
    ...
    if (isleft) {
        if (lastItem) {
            ...
            lastItem.transform = CGAffineTransformMakeScale(1 + (_scaleRate *percent),1 + (_scaleRate *percent));
        }
        [nextItem setTitleColor:lastColor forState:UIControlStateNormal];
        nextItem.transform = CGAffineTransformMakeScale(1 + (1-percent)*_scaleRate,1 + (1-percent)*_scaleRate);
    }else{
        if (lastItem) {
            [lastItem setTitleColor:lastColor forState:UIControlStateNormal];
            lastItem.transform = CGAffineTransformMakeScale((1+ _scaleRate) - (_scaleRate * percent),(1+ _scaleRate) - (_scaleRate * percent));
        }
        [nextItem setTitleColor:self.netxColor forState:UIControlStateNormal];
        /* 在原来的基础上缩放（只缩放一次） */
        nextItem.transform = CGAffineTransformMakeScale(1 + percent *_scaleRate,1 + percent *_scaleRate);
    }
}
- (void)getfontSize
{
...
- (void)scrollToItemCenter:(MCItem *)item
{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        self.lineView.bottom = item.bottom - self.lineHeight - lineBottomMargin;
    }];
}
...
{
    if (!_lineView) {
        _lineView = [[UIView alloc]initWithFrame:CGRectMake(_titleButtonWidth/4, self.titleScrollHeight - 2, _titleButtonWidth/2, 2)];
        ...
        [_contentTitles enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            @autoreleasepool{
                ...
                if (idx ==0) {
                    ...
                    item.transform = CGAffineTransformMakeScale(1+weakSelf.scaleRate,1+weakSelf.scaleRate);
