...
#define kOverlayViewWidth   260     ///自定义选择范围框的宽度
...
{
    [self disappear];
    if ([self.delegate respondsToSelector:@selector(imageCropCancelCorpImage:)]) {
        [self.delegate imageCropCancelCorpImage:self];
    }
}
...
#define kButtonWidth 50
    ...
    //overlayView
    //根据宽度找高度
    CGFloat width = kOverlayViewWidth;
    ...
    if (isBaseOnWidth) {
        //上和下
        self.topBlackView.frame = CGRectMake(0, 0, CGRectGetWidth(self.view.frame), CGRectGetMinY(self.overlayView.frame));
        self.bottomBlackView.frame = CGRectMake(0, CGRectGetMaxY(self.overlayView.frame), CGRectGetWidth(self.view.frame), CGRectGetHeight(self.view.bounds)-CGRectGetMaxY(self.overlayView.frame));
    }else{
        //左和右
...
- (void)adjustImageViewFrameAndScrollViewContent
{
    ...
    if (self.imageView.image) {
        ...
        CGFloat leftOff = (CGRectGetWidth(self.view.frame) - CGRectGetWidth(self.overlayView.frame)) / 2.0;
        self.scrollView.contentInset = UIEdgeInsetsMake(CGRectGetMinY(self.overlayView.frame), leftOff, CGRectGetHeight(self.view.bounds)-CGRectGetMaxY(self.overlayView.frame), leftOff);
        ...
        if (imageFrame.size.width >= imageFrame.size.height) {
            float offy = self.scrollView.bounds.size.width - self.scrollView.contentSize.width;
            CGFloat offsetY = offy * 0.5;
            self.scrollView.contentOffset = CGPointMake(-offsetY,-CGRectGetMinY(self.overlayView.frame));
            
        }else{
            float offy = self.scrollView.bounds.size.height - self.scrollView.contentSize.height;
            CGFloat offsetY = offy * 0.5;
            self.scrollView.contentOffset = CGPointMake(-leftOff, -offsetY);
        }
