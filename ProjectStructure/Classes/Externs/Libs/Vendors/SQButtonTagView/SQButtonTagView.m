...
//    }
    ...
    NSString *text = _tagTexts[tag];
    if ([self.selectArray containsObject:text]) {
        [self.selectArray removeObject:text];
...
-(void)setDefaultSelectedArr:(NSArray *)defaultSelectedArr{
    if (_tagTexts.count == 0) {
        NSLog(@"---- error = 默认选中数组需要放到_tagTexts设置的后面 -----");
        return;
    }
    [self.selectArray removeAllObjects];
    [self.selectArray addObjectsFromArray:defaultSelectedArr];
...
//    }
    ...
    for (int i = 0; i < _tagTexts.count; i++) {
        NSString *text = _tagTexts[i];
        UIButton *buttonTag = self.buttonTags[i];
        if ([self.selectArray containsObject:text]) {
            [buttonTag setBackgroundColor:self.selectedBackgroundColor];
            [buttonTag setTitleColor:self.selectedTagTextColor forState:UIControlStateNormal];
            buttonTag.layer.borderColor = self.selectedBackgroundColor.CGColor;
